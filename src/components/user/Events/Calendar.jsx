        ...
        <div className=" rounded-xl border-gray-400 flex flex-col justify-between">
            <div className="my-6 font-semibold flex items-center justify-between">
                ...
                <div>
                    <button
                        onClick={prevMonth}
                        className="text-black bg-red-200 px-4 py-1 rounded-lg "
                    >Prev</button>
                    <button
                        className="text-black bg-red-200 px-4 py-1 rounded-lg  ml-2"
                        onClick={nextMonth}
                    >Next</button>
                </div>
            </div>
            <div className="grid grid-cols-7 w-full">
                ...
                }
                {
                    Array.from({ length: prefixDays }).map((_, index) => (
                        index === 0 ? <CalendarCell current={false} date={prevMonthDays - (prefixDays - 1 - index)} key={index} /> :
                            <CalendarCell current={false} date={prevMonthDays - (prefixDays - 1 - index)} key={index} />
                    ))
                }
                {
                    Array.from({ length: numDays }).map((_, index) => (
                        index === 0 ? <CalendarCell current={true} date={index} events={eventdata[`${index + 1}`]} key={index} /> :

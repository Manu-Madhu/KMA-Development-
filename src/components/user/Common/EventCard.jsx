...
function EventCard({ Name, Title, Description, Location, Picture, isArchived = false }) {
    return (
        <div className={`p-7 relative border overflow-clip flex flex-col w-[96%] h-80 border-gray-400 justify-between rounded-2xl`}>
            <h1 className='text-red-600 font-semibold'>{Name}</h1>
            <h1 className='text-[2.3rem] font-semibold'>{Title}</h1>
            <p className='w-[45%] text-sm font-medium text-gray-600  my-auto'>
                {Description}
            </p>
            <div className={`px-4 py-1 ${isArchived ? "bg-[#E3000F] text-white" : "bg-[#E3000F33]"} w-fit font-medium rounded-full`}>
                <p>
                    {Location}
                </p>
            </div>
            ...
            <div className={`absolute right-7 top-10 flex flex-col items-end ${isArchived ? "hidden" : ""}`}>
                <p className='text-[2.1rem] text-[#E3000F] leading-8 font-semibold'>21</p>
